...
// JavaScript to handle star rating and form submission
document.querySelectorAll('.rating-stars i').forEach(star => {
  star.addEventListener('click', () => {
      document.querySelectorAll('.rating-stars i').forEach(s => s.classList.remove('selected'));
      star.classList.add('selected');
      document.getElementById('review-rating').value = star.getAttribute('data-value');
  });
});
...
document.getElementById("review-form").addEventListener("submit", async function (e) {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Optionally, fetch and display updated reviews from the server
      updateReviews();
    } else {
      alert(result.message);
    }
